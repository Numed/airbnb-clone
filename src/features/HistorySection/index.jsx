...
import { useActiveUser, useOrderedFlights, useOrderedRooms } from "../../store";

const HistorySection = () => {
  const [historySection, setHistorySection] = useState("Flights");
  const orderedFlights = useOrderedFlights(
    (selectore) => selectore.orderedFlights
  );
  const orderedRooms = useOrderedRooms((selectore) => selectore.orderedRooms);
            ...
            {historySection === "Flights" ? (
              <div className="mt-6 w-full h-full grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
                {orderedFlights.length > 0 ? (
                  orderedFlights.map(({ id, img, rating, ratingText, alt }) => (
                    <div
                      key={id}
